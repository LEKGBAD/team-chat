...
const ServerSearch = ({data}:ServerSearchProps) => {
    ...
    const onClick=({id,type}:{id:string,type:"channel"|"member"})=>{
        setOpen(false);
        if(type==="member"){
            return router.push(`/servers/${params?.serverId}/convsersations/${id}`)
        }
        if(type==="channel"){
    ...
    return ( 
        <>
            ...
            >
                ...
                <p className="font-semibold text-zinc-500 text-sm dark:text-zinc-400 
                group-hover:text-zinc-600 dark:group-hover:text-zinc-300 transition">
                    Search
                </p>
                <kbd
                className=" pointer-event-none inline-flex select-none h-5 items-center gap-1 rounded
